...
# print(f' table.py at {current_dir}')
# items = os_e.listdir(current_dir)
# for item in items:
#     print(f'table.py同資料夾內的文件{item}')
import numpy as np
import re
import pickle
        ...
        if record[od] != "" and record[os] != "":
            if index == 1 :
                if x(age_new)-x(record[11]) == 0:
                    ...
                    print(index,x(age_new),x(record[11]))
